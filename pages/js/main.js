...
$(document).ready(function () {
	...
	if($('.ct-sl-list').length){
		const swiper = new Swiper('.ct-sl-list', {
			spaceBetween: 0,
			slidesPerView: 3,
			slidesPerGroup: 1,
			watchOverflow: true,
			navigation: {
				nextEl: '.ct-sl-list-next',
				prevEl: '.ct-sl-list-prev'
			},
			scrollbar: {
				el: '.ct-sl-list-scroll',
				draggble: true,
			},
			breakpoints: {
				1400: {
		...
		/* $('.ct-sl-list').slick({
			...
			arrows: true,
			dots: false,
			prevArrow: '<button type="button" class="slick-prev"><img src="img/prev-btn-slick.svg"></button>',
			nextArrow: '<button type="button" class="slick-next"><img src="img/next-btn-slick.svg"></button>',
			swipeToSlide: true,
...
//});
	...
	if($(window).width() > 1024){
		$('.pg-razdels').mouseenter(function(){
			$(this).find('.pg-razdels__list').slideDown(200);
			$(this).addClass('_open');
		});
		$('.pg-razdels').mouseleave(function(){
			$(this).find('.pg-razdels__list').slideUp(200);
			$(this).removeClass('_open');
		});
	}
	else{
		$('.pg-razdels').click(function(e){
			console.log('123');
